...
const RED: [ColorComponent; 4] = [1.0, 0.0, 0.0, 1.0];
const BLUE: [ColorComponent; 4] = [0.0, 0.0, 1.0, 1.0];
...
fn count_neighbours(x: usize, y: usize, rects: &Vec<Vec<Rect>>) -> (i32,i32){
    let mut red = 0;
    let mut blue = 0;
...
fn change_state(neighbours:(i32,i32), rect: &mut Rect) {
    let sum = neighbours.0 + neighbours.1;
    if sum < 2 || sum > 3 {
        rect.color = BLACK;
    }
    if rect.color == BLACK && sum == 3{
...
fn paint(pos:[f64; 2], key: Key, rects: &mut Vec<Vec<Rect>>) {
    let x = (pos[1]/SIZE) as usize;
    let y = (pos[0]/SIZE) as usize;
    match key {
        Key::R => {rects[x][y].color = RED;
            rects[x+1][y].color = RED;
            rects[x][y+1].color = RED;
            rects[x+1][y+1].color = RED;},

        Key::B => {rects[x][y].color = BLUE;
            rects[x+1][y].color = BLUE;
            rects[x][y+1].color = BLUE;
            rects[x+1][y+1].color = BLUE;},
        _ => {}
    }
}
...
pub fn main_double() {
    ...
    while let Some(e) = events.next(&mut window) {
        ...
        if let Some(button) = e.press_args() {
            match button {
                Button::Keyboard(key) => paint(cursor, key, &mut rects),
                _ => {}
            }
        }
